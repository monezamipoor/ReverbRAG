...
class RAFDataparserOutputs(NeRAFDataparserOutputs):
    ...
    def __init__(self, audios_filenames, microphone_poses, source_poses, source_rotations, scene_box, references_filenames, full_reference_filenames):
        super().__init__(audios_filenames, microphone_poses, source_poses, scene_box, source_rotations=source_rotations)
        self.references_filenames = references_filenames
        self.full_reference_filenames = full_reference_filenames
...
class RAFDataParser(NeRAFDataParser):
    ...
    def _generate_dataparser_outputs(self, split: str = "train", **kwargs: Optional[Dict]) -> RAFDataparserOutputs:
        ...
        if split == 'inference': # special case for inference
            ...
            full_reference_files = all_files["reference"][0]
            
            if split == "reference":
                poses = self._process_poses(full_reference_files)
                return RAFDataparserOutputs(
                audios_filenames=full_reference_files,
                ...
                scene_box=None,
                references_filenames=None,
                full_reference_filenames=None
                )
            if split == 'train':
                split_files = all_files['train'][0]
            elif split == 'val':
                split_files = all_files['validation'][0]
            else:
                split = "test" 
                split_files = all_files['test'][0]
    
            poses = self._process_poses(split_files)
            
        # Process references
        refs_file = os.path.join(self.config.data, "metadata", "references.json")
        with open(refs_file) as f:
            all_refs = json.load(f)
        split_ref_dict = all_refs.get(split, {})
        references_filenames = []
        for mid in split_files:
            key = str(mid)
            if key not in split_ref_dict:
                raise ValueError(f"No references for '{key}' in split '{split}'")
            references_filenames.append(split_ref_dict[key])

        ref_poses   = self._process_poses(full_reference_files)
        # test: aabb based on poses['mic_pose']
        all_mic = np.concatenate([poses['mic_pose'], ref_poses['mic_pose']], axis=0)
        aabb = np.stack([ all_mic.min(axis=0), all_mic.max(axis=0) ], axis=0)
        # add 1m margin 
        aabb[0] -= 1
        aabb[1] += 1
        ...
        dataparser_outputs = RAFDataparserOutputs(
            ...
            source_rotations=torch.from_numpy(poses['rot']),    
            scene_box=scene_box, 
            references_filenames=references_filenames,
            full_reference_filenames=full_reference_files,
        )
