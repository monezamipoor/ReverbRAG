...
class RAFDataset(Dataset):
    ...
    def __init__(self, dataparser_outputs: RAFDataparserOutputs, 
                 ...
                 mean = None,
                 std = None,
                 reference_num: int = 0,           # ← how many references to fetch (0 = off)
        ...
        fr = dataparser_outputs.full_reference_filenames or []
        self.full_reference_filenames = fr
        self.ref_str2idx = {fid: i for i, fid in enumerate(fr)}
        # NEW: quick membership check for “is this sample in the reference bank?”
        self._ref_set = set(fr)                     # empty set if None/[]
        self._is_reference_split = (mode == "reference")
    ...
    # Put this inside RAFDataset (or module-level if you prefer)
    def _force_full_stft_len(self, stft: torch.Tensor) -> torch.Tensor:
        """
        stft: (1, F, T) complex or real tensor.
        Returns (1, F, self.max_len) by truncating/padding last dim.
        Uses constant pad with the min value to avoid bias.
        """
        T = stft.shape[-1]
        if T > self.max_len:
            ...
            # choose a stable pad value; if complex, use real min and cast
            if torch.is_complex(stft):
                min_val = stft.real.min()
                min_val = torch.complex(min_val, torch.zeros((), device=stft.device, dtype=stft.dtype))
            else:
                min_val = stft.min()
            stft = torch.nn.functional.pad(stft, (0, pad_T), mode='constant', value=min_val)
        return stft

    
    def get_data(self, audio_idx: int):
        """Returns either a single STFT slice (1,F) or full STFT (1,F,T) depending on split."""
        stft_id, stft_tp = self.get_id_tmp(audio_idx)
        audio_filename = self._dataparser_outputs.audios_filenames[stft_id] # e.g. "026564"
        is_in_references = (audio_filename in self._ref_set) or self._is_reference_split
        ...
        # (F, T) → unsqueeze to (1, F, T)
        stft = self.transform_stft_torch(torch.tensor(data)).unsqueeze(0)

        if self.mode == "reference":
            # >>> Reference split: keep FULL STFT in data <<<
            stft = self._force_full_stft_len(stft)
            stft = torch.log(torch.abs(stft) + 1e-3)     # (1, F, T)
            data_field = stft
            time_query = -1
        else:
            # Non-reference: keep a single time-slice like before
            if stft.shape[2] > 0 and (stft_tp < stft.shape[2]):
                stft_slice = stft[:, :, stft_tp]         # (1, F)
                data_field = torch.log(torch.abs(stft_slice) + 1e-3)
            else:
                min_value = torch.min(stft)
                stft_slice = torch.ones(stft.shape[0], stft.shape[1]) * min_value
                data_field = torch.log(torch.abs(stft_slice) + 1e-3)
            time_query = stft_tp
        ...
        # references idx
        if self.reference_num > 0 and self._dataparser_outputs.references_filenames:
            raw = self._dataparser_outputs.references_filenames[stft_id][:self.reference_num]
            reference_idx = [self.ref_str2idx[rid] for rid in raw]
            reference_idx = torch.tensor(reference_idx, dtype=torch.long)
        else:
            reference_idx = []

        return {
            "audio_idx": stft_id,
            "data": data_field,             # (1,F,T) for reference; (1,F) otherwise
            "time_query": time_query,       # -1 for reference
            "rot": source_rotations,
            "mic_pose": microphone_pose,
            "source_pose": source_pose,
            "reference_idx": reference_idx,
            "is_in_reference_bank": torch.tensor(is_in_references, dtype=torch.bool),
        }


        
    def get_data_eval(self, audio_idx: int):
        """Returns the STFT and waveform. Full STFT only for reference split (and normal eval-image)."""
        ...
        stft = self.transform_stft_torch(torch.tensor(data)).unsqueeze(0)  # (1,F,T)

        # pad/truncate only when we intend to keep full STFT
        keep_full = (self.mode == "reference") or (self.mode in ["eval", "eval_image"])  # preserve old eval-image behavior
        if keep_full:
            if stft.shape[2] > self.max_len:
                stft = stft[:, :, :self.max_len]
            else:
                min_value = stft.min()
                pad_T = self.max_len - stft.shape[2]
                if pad_T > 0:
                    stft = torch.nn.functional.pad(stft, (0, pad_T), 'constant', min_value)
            stft = torch.log(torch.abs(stft) + 1e-3)     # (1, F, T)
            data_field = stft
        else:
            # Non-reference & not eval-image: behave like get_data slice for symmetry if ever used
            stft_id, stft_tp = self.get_id_tmp(audio_idx)
        ...
        waveform = torch.from_numpy(data).unsqueeze(0)   # (1, T_time)
        ...
        if self.reference_num > 0 and self._dataparser_outputs.references_filenames:
            raw = self._dataparser_outputs.references_filenames[audio_idx][:self.reference_num]
            reference_idx = [self.ref_str2idx[rid] for rid in raw]
            reference_idx = torch.tensor(reference_idx, dtype=torch.long)
        else:
            reference_idx = []
        ...
        return {
            "audio_idx": audio_idx,
            "data": data_field,             # (1,F,T) for reference (and eval-image); slice otherwise
            "waveform": waveform,
            "rot": source_rotations,
            "mic_pose": microphone_pose,
            "source_pose": source_pose,
    ...
    def get_time_index(self, idx: int) -> int:
        return idx % self.max_len
