...
import numpy as np
import torchaudio
from NeRAF.NeRAF_helper import SpectralLoss, compute_t60, evaluate_edt, evaluate_clarity, Envelope_distance, edc_distance_np  
...
class RAFEvaluator(object):
    ...
    def get_full_metrics(self, mag_prd, mag_gt, wav_gt_ff, wav_pred_istft, wav_gt_istft,  log_prd, log_gt):#, gl=False):
        ...
        # MSE
        mse = float(np.mean((wav_gt_ff - wav_prd) ** 2))

        # --- EDC (per-channel) ---
        n_ch = wav_gt_ff.shape[0]
        edc_vals = []
        for c in range(n_ch):
            edc_c = edc_distance_np(
                pred_wav=wav_prd[c],
                gt_wav=wav_gt_ff[c],
                T_target=60,
                normalize=True
            )
            edc_vals.append(edc_c)
        edc_mean = float(np.mean(edc_vals))
    ...
    def get_stft_metrics(self,mag_prd,mag_gt,gl=False):
        ...
        mag_l2 = torch.norm(mag_prd - mag_gt, p='fro')
