...
class FieldHeadConfig:
    mode: HeadMode = "refine" # "stft", "query", "default", "refine"
...
class NeRAFAudioSoundField(nn.Module):
    ...
    def __init__(
        ...
        if self.mode == "refine":   
            ...
            if self.cfg.mask_predictor == "mlp":
                ...
                self.refine_mask_film_predictor = nn.Linear(self.W , 2 * self.W)
                self.refine_mask_out = nn.Linear(self.W, BANDS)
    ...
    def _predict_mask_bands(self, q_W: torch.Tensor, g_stft: torch.Tensor) -> torch.Tensor:
        ...
        """
        if self.cfg.mask_predictor == "mlp":
            x = torch.cat([q_W, self.stft_to_W(g_stft)], dim=-1)  # ensure same scale
    ...
    def fuse_with_trunk(
        ...
        if self.cfg.gating == "single":
            token_upscale = self.lora_expand_p(token) if self.cfg.fusion == "attn" else token  # or maybe self.attn_to_W
            ...
            if self.mode != "query" and self.cfg.fusion == "bahdanau": 
    ...
    def stft_output_token(
        ...
        if self.cfg.self_attention_time:
            ...
            if time_index is not None and self.cfg.temporal_scope in ("window", "both"):
