...
from .NeRAF_helper import set_global_seed
import torch
...
class NeRAFPipeline(VanillaPipeline):
    ...
    def __init__(
        ...
        self.config = config
        set_global_seed(self.config.seed if hasattr(self.config, "seed") else 1234)
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if step > self.start_step_audio:
            ...
            # after model_audio_outputs is computed
            if hasattr(self.audio_model.field, "_last_alpha") and self.audio_model.field._last_alpha is not None:
                a = self.audio_model.field._last_alpha           # (B,1,1) detached
                gate_value = float(a.mean().cpu().item())
                wandb.log({"gate": gate_value}, step=step)
    ...
    def get_eval_image_metrics_and_images(self, step: int):
        ...
        if step > self.start_step_audio:
            self.audio_datamanager.eval_image_count = 0
        ...
        with Progress(
            ...
            if step > self.start_step_audio:
                ...
                for i in range(num_stft):
                    ...
                    _, batch = self.audio_datamanager.next_eval_image(step)
                    ...
                    if output_path is not None:
                        ...
                        # build a clean metadata dict
                        meta = {
                            "audio_idx":   idx,
                            "data":        batch["data"].detach().cpu().numpy(),
                            "waveform":    batch["waveform"].detach().cpu().numpy(),
                            "rot":         batch["rot"].detach().cpu().numpy(),
                            "mic_pose":    batch["mic_pose"].detach().cpu().numpy(),
                            "source_pose": batch["source_pose"].detach().cpu().numpy(),
                        }

                        # your STFT prediction
                        meta["pred_stft"] = outputs["raw_output"] \
                            .permute(1, 2, 0) \
                            .detach().cpu().numpy()

                        # zero-padded filename
                        idx_str = f"{meta['audio_idx']:06d}"
                        audio_path = os.path.join(output_path, f"eval_{idx_str}.npy")

                        # save dict (use allow_pickle=True on load)
                        np.save(audio_path, meta)
